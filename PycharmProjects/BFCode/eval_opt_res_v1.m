function [score,wRes,simGlucose,ivcSig,ivcSigF,struttura] = eval_opt_res_v1(x,struttura,profile,sim_set,sim_dist,ivc_dist,ivc,options_replay,profile_replay)
    ...
    struttura.model_x0  = model_x0;
    struttura.model_xf  = sim.model_xf;
    struttura.dosekempt = sim.dosekempt;
    struttura.lastMeal  = sim.lastMeal;
    
    ivcSigF = sim.ivcSigF;
    
    %% Cost function
    
    score = sum(((simGlucose'-profile.glucose.values)./(profile.glucose.values*0.125)).^2) + ...
           (x_ident-sim_dist.mup)'/sim_dist.sigmap*(x_ident-sim_dist.mup) + ...
           (x_ne-ivc_dist.mu)'/ivc_dist.sigma*(x_ne-ivc_dist.mu);
    
    wRes = (simGlucose'-profile.glucose.values)./(profile.glucose.values*0.125);
